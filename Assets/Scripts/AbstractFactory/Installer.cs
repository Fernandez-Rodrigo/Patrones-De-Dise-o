using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Instalador : MonoBehaviour
{
    /*[SerializeField] private HeroConfig heroConfiguration;
    [SerializeField] private WeaponConfig weaponConfiguration;

    private void Start()
    {
        var heroFactory = new HeroSpawnerFactory(Instantiate(heroConfiguration));
        var weaponFactory = new WeaponFactory(Instantiate(weaponConfiguration));

        //var consumerGameObject = new GameObject();

        // var consumer = consumerGameObject.AddComponent<SpawnFacade>();
        var consumer = FindObjectOfType<SpawnFacade>();
        consumer.Configure(new CharacterFactory(heroFactory, weaponFactory));

    }
    */
    
}
